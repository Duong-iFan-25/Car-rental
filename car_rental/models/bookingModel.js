...
// Dừng transaction và trả về nguyên nhân không thể xử lý.
async function stopTransaction(connection, reason) {
    await connection.rollback()
    return { success: false, reason: reason }
}

// Thêm đơn trong transaction để hai request đồng thời không thể tạo lịch trùng.
async function createBooking(user_id, car_id, start_date, end_date, note) {
    ...
    try {
        ...
        // Khóa xe và người dùng để các yêu cầu cùng xe hoặc cùng khách phải chờ nhau.
        ...
        // Tính tiền bằng giá mới nhất của xe sau khi hàng xe đã được khóa.
        const [dayRows] = await connection.query(
            "SELECT GREATEST(DATEDIFF(?, ?), 1) AS rental_days",
            [end_date, start_date]
        )
        const total_price = Number(dayRows[0].rental_days) * Number(cars[0].price_per_day)
...
// Tự hoàn thành đơn đã qua ngày trả và cập nhật trạng thái hiện tại của xe.
...
// Lấy toàn bộ đơn cùng thông tin khách hàng và xe cho trang admin.
...
// Duyệt đơn trong transaction và kiểm tra lại lịch trước khi cập nhật.
async function approveBooking(id) {
    ...
    try {
        ...
        // Đọc trước để biết xe và người dùng cần khóa.
...
// Từ chối một đơn đang chờ duyệt.
