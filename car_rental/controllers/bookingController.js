...
async function store(req, res) {
  try {
    ...
    if (!start_date || !end_date) {
      return showBookingError(
        res,
        car,
        "Vui lòng chọn đầy đủ ngày thuê và ngày trả xe",
      );
    }
    ...
    if (!isValidDate(start_date) || !isValidDate(end_date)) {
      return showBookingError(
        res,
        car,
        "Ngày thuê hoặc ngày trả xe không hợp lệ",
      );
    }
    ...
    if (start_date < getTodayString()) {
      return showBookingError(
        res,
        car,
        "Ngày thuê không được nằm trong quá khứ",
      );
    }
    ...
    if (end_date < start_date) {
      return showBookingError(
        res,
        car,
        "Ngày trả xe không được trước ngày thuê xe",
      );
    }
    ...
    if (!result.success) {
      ...
      if (result.reason === "user_conflict") {
        return showBookingError(
          res,
          car,
          "Bạn đã có đơn thuê trùng với khoảng thời gian này",
        );
      }
